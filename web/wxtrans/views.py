...
from web.wxtrans.utils import utils_show_trans, utils_add_trans, utils_show_flow, utils_modify_trans, \
...
@mod.route('/modify_trans', methods=['POST'])
def views_modify_trans():
    req_data = request.get_data()
    result = {'data': {}, 'code': 500}
    if req_data:
        data = json.loads(req_data.decode('utf-8'))
        result['data'] = utils_modify_trans(data)
        result['code'] = 200
    return json.dumps(result)
...
def views_delete_trans():
    ...
    if req_data:
        data = json.loads(req_data.decode('utf-8'))
        result['data'] = utils_delete_trans(data)
        result['code'] = 200
    return json.dumps(result)
